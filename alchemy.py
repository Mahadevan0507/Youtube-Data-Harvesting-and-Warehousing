...
def channelstable(channel_name_s):
    ...
    # Fetch existing channel names
    query_1 = "SELECT Channel_Name FROM channels"
    cursor.execute(query_1)
    existing_channel_names = [row[0] for row in cursor.fetchall()]

    if channel_name_s in existing_channel_names:
        news = f"Your provided channel {channel_name_s} already exists in the database."
        return news
    else:
        ...
        for row in df_single_channel.iterrows():
            insert_query = '''INSERT INTO channels(Channel_Name, Channel_Id, Subscribers, Views, Total_Videos,
...
def videostable(channel_name_s):
